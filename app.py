                            ...
                            if "RECOLHER" in str(row[0]).upper():
                                for i, cell in enumerate(row):
                                    if i < len(headers):
                                        ...
                                        if len(n) > 1:
                                            mapa_final[n] = cls.limpar_valor(cell)
                                break
...
st.markdown(
    ...
    .topo-app {
        background: linear-gradient(180deg, rgba(18, 24, 38, 0.96) 0%, rgba(15, 23, 42, 0.96) 100%);
...
st.markdown(
    """
    <div class="topo-app">
        <span class="topo-tag">JS Energy • Importação e Exportação</span>
        <h1>PLANILHA DE ESTIMATIVAS</h1>
        <p>Geração, edição e exportação de estimativas de custos em PDF</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
st.markdown("<div class='secao-label'>Arquivo de origem</div>", unsafe_allow_html=True)
...
if uploaded_file:
    ...
    with col_info:
        ...
        fob = st.number_input(
            "Valor FOB (BRL)",
            value=st.session_state.pdf_map.get("FOB", 0.0),
            format="%.2f",
        )
        frete = st.number_input(
            "Valor do Frete (BRL)",
            value=st.session_state.pdf_map.get("FRETE", 0.0),
            format="%.2f",
        )
